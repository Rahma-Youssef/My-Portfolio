  ...
  return (
    <div className="flex justify-center items-center w-full">
      <motion.button
        ...
        className={`relative flex items-center border-2 rounded-full hover:scale-95 active:scale-95 overflow-hidden transition-all
          ${isDownloading ? 'cursor-wait border-[#D8728E]' : 'cursor-pointer border-[#D8728E]'}`}
        animate={{
          width: isDownloading ? 56 : 180,
        ...
        <motion.div
          className="h-14 w-14 rounded-full bg-[#c23d60] flex justify-center items-center relative shadow-lg z-10"
          ...
          {/* Progress fill */}
          <motion.div
            className="absolute top-0 left-0 w-full bg-pink-800 rounded-full"
            initial={{ height: '0%' }}
        ...
        <AnimatePresence>
          {!isDownloading && (
            <motion.span
              className="ml-2 text-black dark:text-white text-[15px] font-semibold   select-none z-10"
              initial={{ opacity: 1 }}
              exit={{ opacity: 0 }}
